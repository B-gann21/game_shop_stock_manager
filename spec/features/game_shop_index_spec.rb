...
RSpec.describe 'GameShop', type: :feature do
  ...
  describe '/game_shops/:id' do
    it 'displays the matching GameShop and attibutes' do
      ...
      expect(page).to have_content("Name: #{shop1.name}")
      ...
      expect(page).to_not have_content("Overstocked: #{shop2.overstocked}")
      expect(page).to_not have_content("Stock limit: #{shop2.stock_limit}")
      expect(page).to_not have_content("Created at: #{shop2.created_at}")
      expect(page).to_not have_content("Updated at: #{shop2.updated_at}")
    end
  end
end
